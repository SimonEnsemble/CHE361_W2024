...
consider our continuous flow bioreactor from lecture. suppose the incoming feed is sterile ($b_f(t)=0$ in our notes) and the concentration of substrate in the feed is constant at $x_f(t)=\bar{x_f}$. 
"
...
!!! note
	we are doing this to set up for the next steps, of (i) finding $\bar{x}$ and $\bar{b}$ and (ii) linearizing the model. so, later on, be sure to use these functions `g_b` and `g_x` you code up here.
"
...
🐛 derive the linearized transfer function model that determines the dynamics of how $F(s):=\mathcal{L}[F^*(t)]$ affects $B(s):=\mathcal{L}[b^*(t)]$. substitute the numerical values you obtained from above. 
